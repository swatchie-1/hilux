...
###   This script attempts to download the signature file SHA256SUMS.asc from hilux.org
...
WORKINGDIR="/tmp/hilux"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://hilux.org/bin/"
VERSIONPREFIX="hilux-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'hilux-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Hilux Core binary release signing key installed?"
   fi
