...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        // NOTE: unlike in hilux, we need to pass PREVIOUS block height here
...
// ***TODO*** ScanHash is not yet used in Hilux
//
...
// ***TODO*** that part changed in hilux, we are using a mix with old one here for now
void static HiluxMiner(const CChainParams& chainparams, CConnman& connman)
{
    LogPrintf("HiluxMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("hilux-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("HiluxMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("HiluxMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("HiluxMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("HiluxMiner -- runtime error: %s\n", e.what());
        return;
    }
}

void GenerateHiluxs(bool fGenerate, int nThreads, const CChainParams& chainparams, CConnman& connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HiluxMiner, boost::cref(chainparams), boost::ref(connman)));
}
