...
struct for_each_chunk_i_visitor
{
    ...
    template <typename Pos, typename Fn>
    friend void visit_relhiluxd(this_t, Pos&& pos,
                              size_t first, size_t last,
                              Fn&& fn)
    {
        // we are going towards *two* indices, so we need to do the
        // relhiluxd as a special case to correct the second index
        if (first < last)  {
            auto f = pos.index(first);
...
struct for_each_chunk_p_i_visitor
{
    ...
    template <typename Pos, typename Fn>
    friend bool visit_relhiluxd(this_t, Pos&& pos,
                              size_t first, size_t last,
                              Fn&& fn)
    {
        // we are going towards *two* indices, so we need to do the
...
struct equals_visitor
{
    ...
    struct rrb
    {
                               ...
                               Node* rootl, shift_t shiftl, size_t sizel)
        {
            ...
            return shiftl == posr.shift()
                ? visit_maybe_relhiluxd_sub(rootl, shiftl, sizel,
    ...
    friend bool visit_relhiluxd(this_t, PosL&& posl, PosR&& posr,
                              Iter&& first, size_t idx)
    {
        auto nl = posl.node();
        auto nr = posr.node();
    ...
    template <typename PosL, typename PosR, typename Iter>
    friend std::enable_if_t<is_relhiluxd_v<PosR>, bool>
    visit_regular(this_t, PosL&& posl, PosR&& posr, Iter&& first, size_t idx)
    {
        return visit_relhiluxd(this_t{}, posl, posr, first, idx);
    }
...
struct update_visitor
{
    ...
    template <typename Pos, typename Fn>
    friend node_t* visit_relhiluxd(this_t, Pos&& pos, size_t idx, Fn&& fn)
    {
        auto offset  = pos.index(idx);
        auto count   = pos.count();
        auto node    = node_t::make_inner_sr_n(count, pos.relhiluxd());
        try {
...
struct dec_visitor
{
    ...
    friend void visit_relhiluxd(this_t, Pos&& p)
    {
...
{
    visit_maybe_relhiluxd_sub(node, shift, size, dec_visitor());
}

template <typename NodeT>
void dec_relhiluxd(NodeT* node, shift_t shift)
{
    make_relhiluxd_pos(node, shift, node->relhiluxd()).visit(dec_visitor());
}
...
struct get_mut_visitor
{
    ...
    template <typename Pos>
    friend value_t& visit_relhiluxd(this_t, Pos&& pos, size_t idx,
        ...
        if (node->can_mutate(e)) {
            ...
            try {
                ...
                *location = new_node;
                ...
                dec_relhiluxd(new_node, pos.shift());
                throw;
            }
        }
    }
...
struct push_tail_mut_visitor
{
    ...
    template <typename Pos>
    friend node_t* visit_relhiluxd(this_t, Pos&& pos, edit_t e, node_t* tail, count_t ts)
    {
        auto node        = pos.node();
        ...
        if (mutate) {
            ...
            auto relhiluxd = node->ensure_mutable_relhiluxd_n(e, new_idx);
            node->inner()[new_idx]  = new_child;
            relhiluxd->d.sizes[new_idx] = pos.size() + ts;
            relhiluxd->d.count = count;
            return node;
        } else {
            try {
                auto count    = new_idx + 1;
                auto new_node = node_t::copy_inner_r_e(e, pos.node(), new_idx);
                auto relhiluxd  = new_node->relhiluxd();
...
struct push_tail_visitor
{
    ...
    template <typename Pos>
    friend node_t* visit_relhiluxd(this_t, Pos&& pos, node_t* tail, count_t ts)
    {
        auto level       = pos.shift();
        auto idx         = pos.count() - 1;
        auto children    = pos.size(idx);
        ...
        try {
            ...
            auto new_relhiluxd = new_parent->relhiluxd();
            new_parent->inner()[new_idx] = new_child;
            new_relhiluxd->d.sizes[new_idx] = pos.size() + ts;
            new_relhiluxd->d.count = count;
            return new_parent;
        } catch (...) {
            auto shift = pos.shift();
            auto size  = new_idx == idx ? children + ts : ts;
...
struct dec_right_visitor
{
    ...
    template <typename Pos>
    friend void visit_relhiluxd(this_t, Pos&& p, count_t idx)
    {
        using node_t = node_type<Pos>;
        auto node = p.node();
        if (node->dec()) {
...
struct slice_right_mut_visitor
{
    ...
    template <typename PosT>
    friend result_t visit_relhiluxd(this_t, PosT&& pos, size_t last, edit_t e)
    {
        ...
        if (Collapse && idx == 0) {
            ...
            try {
                if (next) {
                    if (mutate) {
                        auto nodr = node->ensure_mutable_relhiluxd_n(e, idx);
                    ...
                    if (mutate) {
                        pos.each_right(dec_visitor{}, idx + 1);
                        node->ensure_mutable_relhiluxd_n(e, idx)->d.count = idx;
                        return { pos.shift(), node, ts, tail };
                    } else {
                        auto newn = node_t::copy_inner_r_e(e, node, idx);
                        if (Mutating) pos.visit(dec_visitor{});
...
struct slice_right_visitor
{
    ...
    template <typename PosT>
    friend result_t visit_relhiluxd(this_t, PosT&& pos, size_t last)
    {
        auto idx = pos.index(last);
        if (Collapse && idx == 0) {
...
struct slice_left_mut_visitor
{
    ...
    using edit_t = typename NodeT::edit_t;
    using value_t = typename NodeT::value_t;
    using relhiluxd_t = typename NodeT::relhiluxd_t;
    ...
    template <typename PosT>
    friend result_t visit_relhiluxd(this_t, PosT&& pos, size_t first, edit_t e)
    {
        ...
        if (Collapse && pos.shift() > BL && idx == pos.count() - 1) {
            ...
            using std::get;
            auto newn = mutate
                ? (node->ensure_mutable_relhiluxd(e), node)
    ...
    friend result_t visit_regular(this_t, PosT&& pos, size_t first, edit_t e)
    {
        ...
        if (Collapse && pos.shift() > BL && idx == pos.count() - 1) {
            ...
            using std::get;
            // if possible, we convert the node to a relhiluxd one
            // simply by allocating a `relhiluxd_t` size table for
            // it... maybe some of this magic should be moved as a
            // `node<...>` static method...
            auto newcount = count - idx;
            auto newn = mutate
                ? (node->impl.d.data.inner.relhiluxd = new (
                       node_t::heap::allocate(
                           node_t::max_sizeof_relhiluxd,
                           norefs_tag{})) relhiluxd_t,
                   node)
                : node_t::make_inner_r_e(e);
            auto newr = newn->relhiluxd();
            try {
                ...
                if (!mutate) node_t::delete_inner_r_e(newn);
                    ...
                    // attempting to relax...
                    node_t::heap::deallocate(node_t::max_sizeof_relhiluxd,
                                             node->impl.d.data.inner.relhiluxd);
                    node->impl.d.data.inner.relhiluxd = nullptr;
                }
                throw;
            }
        }
...
struct concat_center_pos
{
    ...
    void each_sub(Visitor v, Args&& ...args)
    {
        if (shift_ == BL) {
            ...
            for (auto i = count_t{0}; i < count_; ++i)
                make_relhiluxd_pos(nodes_[i], shift_ - B, nodes_[i]->relhiluxd()).visit(v, args...);
        }
    }

    relhiluxd_pos<Node> realize() &&
    {
        if (count_ > 1) {
            try {
                auto result = node_t::make_inner_r_n(count_);
                auto r      = result->relhiluxd();
                r->d.count    = count_;
            ...
            assert(shift_ >= B + BL);
            return { nodes_[0], shift_ - B, nodes_[0]->relhiluxd() };
        }
    }

    relhiluxd_pos<Node> realize_e(edit_t e)
    {
        if (count_ > 1) {
    ...
    void merge_inner(Pos&& p)
    {
        ...
        if (!to_ && *curr_ == from_count) {
            ...
            do {
                ...
                auto sizes   = to_->relhiluxd()->d.sizes;
                ...
                if (*curr_ == to_offset_) {
                    to_->relhiluxd()->d.count = to_offset_;
...
struct concat_trees_left_visitor
{
    ...
    { return visit_maybe_relhiluxd_sub(
            args...,
...
struct concat_merger_mut
{
    ...
    {
        if (candidate) {
            candidate->ensure_mutable_relhiluxd_e(candidate_e, ec);
    ...
    void add_child(node_t* p, size_t size)
    {
        ...
        if (count_ == branches<B>) {
            parent->relhiluxd()->d.count = count_;
            assert(result_.count_ < result_t::max_children);
            n_ -= branches<B>;
            if (candidate_) {
                parent = candidate_;
                parent->ensure_mutable_relhiluxd_e(candidate_e_, ec_);
                candidate_ = nullptr;
            } else
        ...
        if (!to_ && *curr_ == from_count) {
            ...
            do {
                if (!to_) {
                    if (from_mutate) {
                        node_t::ownee(from) = ec_;
                        from->ensure_mutable_relhiluxd_e(e, ec_);
                        to_ = from;
                    } else {
    ...
    {
        assert(!to_);
        result_.nodes_[result_.count_ - 1]->relhiluxd()->d.count = count_;
        return result_;
    }
