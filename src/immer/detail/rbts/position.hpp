...
struct relhiluxd_pos
{
    ...
    node_t* node_;
    shift_t shift_;
    relhiluxd_t* relhiluxd_;

    count_t count() const { return relhiluxd_->d.count; }
    node_t* node()  const { return node_; }
    size_t  size()  const { return relhiluxd_->d.sizes[relhiluxd_->d.count - 1]; }
    shift_t shift() const { return shift_; }
    count_t subindex(size_t idx) const { return index(idx); }
    relhiluxd_t* relhiluxd() const { return relhiluxd_; }

    size_t size_before(count_t offset) const
    { return offset ? relhiluxd_->d.sizes[offset - 1] : 0; }
    ...
    {
        assert(size_before_hint == size_before(offset));
        return relhiluxd_->d.sizes[offset] - size_before_hint;
    }

    count_t index(size_t idx) const
    {
        auto offset = idx >> shift_;
        while (relhiluxd_->d.sizes[offset] <= idx) ++offset;
        return offset;
    }
                    ...
                    size_t* sizes)
    {
        ...
        auto these = relhiluxd_->d.sizes + offset;
    ...
    template <typename Visitor, typename... Args>
    void each(Visitor v, Args&&... args)
    { each_left(v, relhiluxd_->d.count, args...); }
    ...
    bool each_pred(Visitor v, Args&&... args)
    {
        ...
        if (shift_ == BL) {
            for (auto i = count_t{0}; i < n; ++i) {
                ...
                if (!make_leaf_sub_pos(p[i], relhiluxd_->d.sizes[i] - s)
                    .visit(v, args...))
                    return false;
                s = relhiluxd_->d.sizes[i];
            }
        } else {
            auto ss = shift_ - B;
            for (auto i = count_t{0}; i < n; ++i) {
                if (!visit_maybe_relhiluxd_sub(p[i], ss, relhiluxd_->d.sizes[i] - s,
                                             v, args...))
                    return false;
                s = relhiluxd_->d.sizes[i];
            }
    ...
    bool each_pred_i(Visitor v, count_t i, count_t n, Args&&... args)
    {
        if (shift_ == BL) {
            auto p = node_->inner();
            auto s = i > 0 ? relhiluxd_->d.sizes[i - 1] : 0;
            for (; i < n; ++i) {
                IMMER_PREFETCH(p + i + 1);
                if (!make_leaf_sub_pos(p[i], relhiluxd_->d.sizes[i] - s)
                    .visit(v, args...))
    ...
    {
        assert(start > 0);
        assert(start <= relhiluxd_->d.count);
        auto s = relhiluxd_->d.sizes[start - 1];
        auto p = node_->inner();
        if (shift_ == BL) {
            for (auto i = start; i < relhiluxd_->d.count; ++i) {
                IMMER_PREFETCH(p + i + 1);
                if (!make_leaf_sub_pos(p[i], relhiluxd_->d.sizes[i] - s)
                    .visit(v, args...))
                    return false;
    ...
    void each_i(Visitor v, count_t i, count_t n, Args&&... args)
    {
        if (shift_ == BL) {
            ...
            for (; i < n; ++i) {
                IMMER_PREFETCH(p + i + 1);
                make_leaf_sub_pos(p[i], relhiluxd_->d.sizes[i] - s)
                    .visit(v, args...);
                s = relhiluxd_->d.sizes[i];
            }
        } else {
            ...
            auto ss = shift_ - B;
            for (; i < n; ++i) {
                visit_maybe_relhiluxd_sub(p[i], ss, relhiluxd_->d.sizes[i] - s,
                                        v, args...);
                s = relhiluxd_->d.sizes[i];
            }
        }
    ...
    template <typename Visitor, typename... Args>
    void each_left_sub(Visitor v, Args&&... args)
    { each_left(v, relhiluxd_->d.count - 1, args...); }
    ...
    {
        assert(offset_hint == index(idx));
        auto left_size = offset_hint ? relhiluxd_->d.sizes[offset_hint - 1] : 0;
                                      ...
                                      Args&&... args)
    {
        ...
        assert(left_size_hint ==
               (offset_hint ? relhiluxd_->d.sizes[offset_hint - 1] : 0));
        auto child     = node_->inner() [offset_hint];
        auto is_leaf   = shift_ == BL;
        auto next_size = relhiluxd_->d.sizes[offset_hint] - left_size_hint;
        auto next_idx  = idx - left_size_hint;
        return is_leaf
            ? make_leaf_sub_pos(child, next_size).visit(
                v, next_idx, args...)
            : visit_maybe_relhiluxd_sub(child, shift_ - B, next_size,
                                      v, next_idx, args...);
    }
                               ...
                               Args&&... args)
    {
        ...
        return is_leaf
            ...
            : visit_maybe_relhiluxd_sub(child, shift_ - B, child_size_hint,
                                      v, args...);
    }
    ...
    decltype(auto) last_sub(Visitor v, Args&&... args)
    {
        auto offset     = relhiluxd_->d.count - 1;
        auto child      = node_->inner() [offset];
        auto child_size = size(offset);
        auto is_leaf    = shift_ == BL;
        return is_leaf
            ? make_leaf_sub_pos(child, child_size).visit(v, args...)
            : visit_maybe_relhiluxd_sub(child, shift_ - B, child_size, v, args...);
    }
    ...
    {
        auto child      = node_->inner() [0];
        auto child_size = relhiluxd_->d.sizes[0];
        auto is_leaf    = shift_ == BL;
        return is_leaf
            ? make_leaf_sub_pos(child, child_size).visit(v, args...)
            : visit_maybe_relhiluxd_sub(child, shift_ - B, child_size, v, args...);
    ...
    decltype(auto) first_sub_inner(Visitor v, Args&&... args)
    {
        ...
        auto child      = node_->inner() [0];
        auto child_size = relhiluxd_->d.sizes[0];
        return visit_maybe_relhiluxd_sub(child, shift_ - B, child_size, v, args...);
    }
    ...
    decltype(auto) visit(Visitor v, Args&& ...args)
    {
        return visit_relhiluxd(v, *this, std::forward<Args>(args)...);
    }
};

template <typename Pos>
using is_relhiluxd = std::is_same<relhiluxd_pos<typename std::decay_t<Pos>::node_t>,
                                std::decay_t<Pos>>;

template <typename Pos>
constexpr auto is_relhiluxd_v = is_relhiluxd<Pos>::value;

template <typename NodeT>
relhiluxd_pos<NodeT> make_relhiluxd_pos(NodeT* node,
                                    shift_t shift,
                                    typename NodeT::relhiluxd_t* relhiluxd)
{
    assert(node);
    assert(relhiluxd);
    assert(shift >= NodeT::bits_leaf);
    return {node, shift, relhiluxd};
}

template <typename NodeT, typename Visitor, typename... Args>
decltype(auto) visit_maybe_relhiluxd_sub(NodeT* node, shift_t shift, size_t size,
                                       Visitor v, Args&& ...args)
{
    assert(node);
    auto relhiluxd = node->relhiluxd();
    if (relhiluxd) {
        assert(size == relhiluxd->d.sizes[relhiluxd->d.count - 1]);
        return make_relhiluxd_pos(node, shift, relhiluxd)
...
#pragma GCC diagnostic pop
    ...
    decltype(auto) descend(Visitor v, size_t idx)
    {
        auto offset    = index(idx);
        auto child     = node_->inner() [offset];
        auto left_size = offset ? relhiluxd_->d.sizes[offset - 1] : 0;
        auto next_idx  = idx - left_size;
        auto r  = child->relhiluxd();
        return r
            ? relhiluxd_descent_pos<NodeT, Shift - B>{child, r}.visit(v, next_idx)
...
template <typename NodeT, bits_t B, bits_t BL>
struct relhiluxd_descent_pos<NodeT, BL, B, BL>
{
    using node_t = NodeT;
    using relhiluxd_t = typename NodeT::relhiluxd_t;
    node_t* node_;
...
template <typename NodeT, typename Visitor, typename... Args>
decltype(auto) visit_maybe_relhiluxd_descent(NodeT* node, shift_t shift,
                                           Visitor v, size_t idx)
{
    ...
    assert(node);
    assert(shift >= BL);
    auto r = node->relhiluxd();
    if (r) {
        switch (shift) {
        case BL + B * 0: return relhiluxd_descent_pos<NodeT, BL + B * 0>{node, r}.visit(v, idx);
        case BL + B * 1: return relhiluxd_descent_pos<NodeT, BL + B * 1>{node, r}.visit(v, idx);
        case BL + B * 2: return relhiluxd_descent_pos<NodeT, BL + B * 2>{node, r}.visit(v, idx);
        case BL + B * 3: return relhiluxd_descent_pos<NodeT, BL + B * 3>{node, r}.visit(v, idx);
        case BL + B * 4: return relhiluxd_descent_pos<NodeT, BL + B * 4>{node, r}.visit(v, idx);
        case BL + B * 5: return relhiluxd_descent_pos<NodeT, BL + B * 5>{node, r}.visit(v, idx);
#if IMMER_DESCENT_DEEP
        default:
            for (auto level = shift; level != endshift<B, BL>; level -= B) {
                auto r = node->relhiluxd();
