...
struct rrbtree
{
    ...
    void traverse(Visitor v, Args&&... args) const
    {
        ...
        if (tail_off) visit_maybe_relhiluxd_sub(root, shift, tail_off, v, args...);
    ...
    void traverse(Visitor v, size_t first, size_t last, Args&&... args) const
    {
        ...
        if (first < tail_off)
            visit_maybe_relhiluxd_sub(root, shift, tail_off, v,
                                    first,
    ...
    bool traverse_p(Visitor v, Args&&... args) const
    {
        ...
        auto tail_size = size - tail_off;
        return (tail_off
                ? visit_maybe_relhiluxd_sub(root, shift, tail_off, v, args...)
    ...
    decltype(auto) descend(Visitor v, size_t idx) const
    {
        ...
        return idx >= tail_off
            ...
            : visit_maybe_relhiluxd_descent(root, shift, v, idx);
    }
    ...
    bool equals(const rrbtree& other) const
    {
        ...
        if (tail_off > 0 && tail_off_other > 0) {
            ...
            // relhiluxd trees that sadly we haven't managed to exercise
            // in tests yet...
            if (other.shift >= shift) {
                if (!visit_maybe_relhiluxd_sub(
    ...
    {
        if (auto r = root->relhiluxd()) {
            auto new_root = make_relhiluxd_pos(root, shift, r)
            ...
            if (new_root)
                ...
                try {
                    ...
                    new_root->relhiluxd()->d.sizes [0] = size;
                    new_root->relhiluxd()->d.sizes [1] = size + tail_size;
                    new_root->relhiluxd()->d.count = 2u;
                } catch (...) {
        ...
        if (auto r = root->relhiluxd()) {
            ...
            if (new_root) {
                ...
                try {
                    ...
                    new_root->inner() [1] = new_path;
                    new_root->relhiluxd()->d.sizes [0] = tail_off;
                    new_root->relhiluxd()->d.sizes [1] = tail_off + tail_size;
                    new_root->relhiluxd()->d.count = 2u;
                    root = new_root;
                    shift += B;
                } catch (...) {
    ...
    region_for(size_t idx) const
    {
        ...
        if (idx >= tail_off) {
            ...
            auto subs = visit_maybe_relhiluxd_sub(
                root, shift, tail_off,
    ...
    rrbtree update(size_t idx, FnT&& fn) const
    {
        ...
        if (idx >= tail_off) {
            ...
            auto new_root  = visit_maybe_relhiluxd_sub(
                root, shift, tail_off,
    ...
    void take_mut(edit_t e, size_t new_size)
    {
        ...
        if (new_size == 0) {
            *this = empty();
            ...
            auto r = visit_maybe_relhiluxd_sub(root, shift, tail_off, v, l, e);
    ...
    rrbtree take(size_t new_size) const
    {
        ...
        if (new_size == 0) {
            ...
            auto r = visit_maybe_relhiluxd_sub(root, shift, tail_off, v, l);
            auto new_shift = get<0>(r);
            auto new_root  = get<1>(r);
            auto new_tail  = get<3>(r);
            if (new_root) {
    ...
    void drop_mut(edit_t e, size_t elems)
    {
        ...
        if (elems == 0) {
            ...
            *this = empty();
            ...
            auto v = slice_left_mut_visitor<node_t>();
            auto r = visit_maybe_relhiluxd_sub(root, shift, tail_off, v, elems, e);
            shift = get<0>(r);
            root  = get<1>(r);
            size -= elems;
            return;
    ...
    rrbtree drop(size_t elems) const
    {
        if (elems == 0) {
            ...
            auto r = visit_maybe_relhiluxd_sub(root, shift, tail_offset(), v, elems);
            auto new_root  = get<1>(r);
            auto new_shift = get<0>(r);
