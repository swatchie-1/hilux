...
struct node
{
    ...
    static constexpr bool embed_relhiluxd = memory::prefer_fewer_bigger_objects;
    ...
    struct relhiluxd_data_t
    {
        count_t count;
        size_t  sizes[branches<B>];
    };

    using relhiluxd_data_with_meta_t =
        combine_standard_layout_t<relhiluxd_data_t,
                                  refs_t,
                                  ownee_t>;

    using relhiluxd_data_no_meta_t =
        combine_standard_layout_t<relhiluxd_data_t>;

    using relhiluxd_t = std::conditional_t<embed_relhiluxd,
                                         relhiluxd_data_no_meta_t,
                                         relhiluxd_data_with_meta_t>;
    ...
    constexpr static std::size_t sizeof_packed_relhiluxd_n(count_t count)
    {
        return immer_offsetof(relhiluxd_t, d.sizes)
            +  sizeof(size_t) * count;
    }

    constexpr static std::size_t sizeof_packed_inner_r_n(count_t count)
    {
        return embed_relhiluxd
            ? sizeof_packed_inner_n(count) + sizeof_packed_relhiluxd_n(count)
    ...
    constexpr static std::size_t max_sizeof_relhiluxd =
        sizeof_packed_relhiluxd_n(branches<B>);
    ...
    constexpr static std::size_t sizeof_relhiluxd_n(count_t n)
    { return keep_headroom ? max_sizeof_relhiluxd : sizeof_packed_relhiluxd_n(n); }
    ...
    relhiluxd_t* relhiluxd()
    {
        ...
        return impl.d.data.inner.relhiluxd;
    }

    const relhiluxd_t* relhiluxd() const
    {
        assert(kind() == kind_t::inner);
        return impl.d.data.inner.relhiluxd;
    }
    ...
    static refs_t& refs(const relhiluxd_t* x) { return auto_const_cast(get<refs_t>(*x)); }
    static const ownee_t& ownee(const relhiluxd_t* x) { return get<ownee_t>(*x); }
    static ownee_t& ownee(relhiluxd_t* x) { return get<ownee_t>(*x); }
    ...
    static node_t* make_inner_n(count_t n)
    {
        ...
        auto p = new (m) node_t;
        p->impl.d.data.inner.relhiluxd = nullptr;
#if IMMER_RBTS_TAGGED_NODE
    ...
    static node_t* make_inner_r_n(count_t n)
    {
        ...
        if (embed_relhiluxd) {
            ...
            try {
                mr = heap::allocate(sizeof_relhiluxd_n(n), norefs_tag{});
            } catch (...) {
        ...
        auto r = new (mr) relhiluxd_t;
        r->d.count = 0;
        p->impl.d.data.inner.relhiluxd = r;
#if IMMER_RBTS_TAGGED_NODE
        p->impl.d.kind = node_t::kind_t::inner;
#endif
        return p;
    ...
    static node_t* make_inner_sr_n(count_t n, relhiluxd_t* r)
    {
        return static_if<embed_relhiluxd, node_t*>(
            [&] (auto) {
    ...
    static node_t* make_inner_r_e(edit_t e)
    {
        ...
        if (embed_relhiluxd) {
            ...
            try {
                mr = heap::allocate(max_sizeof_relhiluxd, norefs_tag{});
            } catch (...) {
        ...
        auto r = new (mr) relhiluxd_t;
        ownee(p) = e;
        static_if<!embed_relhiluxd>([&](auto){ node_t::ownee(r) = e; });
        r->d.count = 0;
        p->impl.d.data.inner.relhiluxd = r;
#if IMMER_RBTS_TAGGED_NODE
        p->impl.d.kind = node_t::kind_t::inner;
    ...
    static node_t* make_inner_sr_e(edit_t e, relhiluxd_t* r)
    {
        return static_if<embed_relhiluxd, node_t*>(
            [&] (auto) {
    ...
    {
        assert(src->kind() == kind_t::inner);
        auto dst = make_inner_sr_e(e, src->relhiluxd());
    ...
    static node_t* do_copy_inner_r(node_t* dst, node_t* src, count_t n)
    {
        assert(dst->kind() == kind_t::inner);
        assert(src->kind() == kind_t::inner);
        auto src_r = src->relhiluxd();
        auto dst_r = dst->relhiluxd();
        inc_nodes(src->inner(), n);
    ...
    static void delete_inner_r_e(node_t* p)
    {
        ...
        static_if<!embed_relhiluxd>([&] (auto) {
            if (node_t::refs(r).dec())
                heap::deallocate(node_t::max_sizeof_relhiluxd, r);
        });
    ...
    {
        return !embed_relhiluxd || relhiluxd();
    }

    relhiluxd_t* ensure_mutable_relhiluxd(edit_t e)
    {
        auto src_r = relhiluxd();
        return static_if<embed_relhiluxd, relhiluxd_t*>(
            [&] (auto) { return src_r; },
            [&] (auto) {
                if (node_t::refs(src_r).unique() ||
                    ...
                    auto dst_r = impl.d.data.inner.relhiluxd =
                        new (heap::allocate(max_sizeof_relhiluxd)) relhiluxd_t;
    ...
    relhiluxd_t* ensure_mutable_relhiluxd_e(edit_t e, edit_t ec)
    {
        auto src_r = relhiluxd();
        return static_if<embed_relhiluxd, relhiluxd_t*>(
            [&] (auto) { return src_r; },
    ...
    relhiluxd_t* ensure_mutable_relhiluxd_n(edit_t e, count_t n)
    {
        auto src_r = relhiluxd();
        return static_if<embed_relhiluxd, relhiluxd_t*>(
            [&] (auto) { return src_r; },
            [&] (auto) {
                if (node_t::refs(src_r).unique() ||
                    ...
                    std::copy(src_r->d.sizes, src_r->d.sizes + n, dst_r->d.sizes);
                    node_t::ownee(dst_r) = e;
                    return impl.d.data.inner.relhiluxd = dst_r;
                }
            });
    }
