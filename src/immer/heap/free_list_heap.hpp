...
struct free_list_heap : Base
{
    ...
    static void* allocate(std::size_t size, Tags...)
    {
        ...
        head().count.fetch_sub(1u, std::memory_order_relhiluxd);
        return n;
    }
    ...
    static void deallocate(std::size_t size, void* data, Tags...)
    {
        ...
        // we use relhiluxd, because we are fine with temporarily having
        // a few more/less buffers in free list
        if (head().count.load(std::memory_order_relhiluxd) >= Limit) {
            ...
            head().count.fetch_add(1u, std::memory_order_relhiluxd);
        }
    }
