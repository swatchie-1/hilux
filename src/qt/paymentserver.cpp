...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the hilux: URI contains a payment request, we are not able to detect the
    ...
    optionsModel(0)
{
    ...
    // on Mac: sent when you click hilux: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start hilux: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling hilux: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in hilux: URIs
    ...
    if (s.startsWith(HILUX_IPC_PREFIX, Qt::CaseInsensitive)) // hilux: URI
    {
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseHiluxURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Hilux address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        // Hilux amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
