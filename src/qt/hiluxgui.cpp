...
const std::string HiluxGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString HiluxGUI::DEFAULT_WALLET = "~Default";

HiluxGUI::HiluxGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    platformStyle(platformStyle)
{
    ...
    QPixmap pixmapLogo(":/images/light/hilux_logo_horizontal");
...
void HiluxGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Hilux address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and hilux: URIs)"));
    ...
    aboutAction = new QAction(tr("&About Hilux"), this);
    aboutAction->setStatusTip(tr("Show information about Hilux"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Hilux"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Hilux addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Hilux addresses"));
    ...
    openAction->setStatusTip(tr("Open a hilux: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Hilux help message to get a list with possible Hilux command-line options"));
...
void HiluxGUI::createToolBars(QWidget* statusbar)
{
    if(walletFrame)
    {
...
void HiluxGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool HiluxGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool HiluxGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void HiluxGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void HiluxGUI::setWalletActionsEnabled(bool enabled)
{
...
void HiluxGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    trayIcon = new QSystemTrayIcon(this);
    QString toolTip = tr("Hilux client") + " " + networkStyle->getTitleAddText();
...
void HiluxGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void HiluxGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void HiluxGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void HiluxGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void HiluxGUI::gotoSettingsPage() 
{
    settingsAction->setChecked(true);
    if(walletFrame) walletFrame->gotoSettingsPage();
}

void HiluxGUI::gotoToolsPage()
{
    toolsAction->setChecked(true);
    if(toolsAction) walletFrame->gotoToolsPage();
}

void HiluxGUI::gotoMasternodePage()
{
    QSettings settings;
    if (settings.value("fShowMasternodesTab").toBool()) {
...
void HiluxGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void HiluxGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void HiluxGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void HiluxGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void HiluxGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Hilux network", "", count));
    } else {
...
void HiluxGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void HiluxGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void HiluxGUI::updateHeadersSyncProgressLabel()
{
...
void HiluxGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void HiluxGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void HiluxGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Hilux"); // default title
...
void HiluxGUI::changeEvent(QEvent *e)
{
...
void HiluxGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void HiluxGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void HiluxGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(HiluxUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void HiluxGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void HiluxGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool HiluxGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool HiluxGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void HiluxGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void HiluxGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void HiluxGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void HiluxGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void HiluxGUI::detectShutdown()
{
...
void HiluxGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void HiluxGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void HiluxGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(HiluxGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void HiluxGUI::subscribeToCoreSignals()
{
...
void HiluxGUI::unsubscribeFromCoreSignals()
{
...
void HiluxGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void HiluxGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<HiluxUnits::Unit> units = HiluxUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const HiluxUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(HiluxUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(HiluxUnits::Unit u, HiluxUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(HiluxUnits::name(u)), this);
...
{
    setText(HiluxUnits::name(newUnits));
}
