...
const char * const BITCOIN_CONF_FILENAME = "hilux.conf";
const char * const BITCOIN_PID_FILENAME = "hiluxd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                if(ptrCategory->count(std::string("hilux"))) {
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty hilux.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override hilux.conf
