...
// which are part of the HILUX repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="hilux-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /hilux-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "hiluxrunners-hilux/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from hiluxrunners-hilux/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'hiluxrunners-hilux/development', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from hiluxrunners-hilux/development"
            }
          }

          if (hasCache) {
            sh "cd /hilux-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /hilux-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /hilux-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /hilux-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /hilux-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /hilux-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /hilux-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
