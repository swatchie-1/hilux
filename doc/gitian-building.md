...
*Setup instructions for a Gitian build of Hilux Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Hilux
...
- [Building Hilux Core](#building-hilux-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-hilux-host/README.md).
...
Clone the git repositories for Hilux Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/hiluxcrypto/hilux
```
...
in the Hilux Core repository under 'Fetch and build inputs' to install sources which require
...
To build Hilux Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Hilux Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/hilux/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/hiluxcrypto/hilux
...
For example:
```bash
URL=https://github.com/crowning-/hilux.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit hilux=${COMMIT} --url hilux=${URL} ../hilux/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit hilux=${COMMIT} --url hilux=${URL} ../hilux/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit hilux=${COMMIT} --url hilux=${URL} ../hilux/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the hilux git repository with the desired tag must both be available locally, and then gbuild must be
...
cd /some/root/path/
git clone https://github.com/hiluxcrypto/hilux-detached-sigs.git

HLXPATH=/some/root/path/hilux.git
SIGPATH=/some/root/path/hilux-detached-sigs.git

./bin/gbuild --url hilux=${HLXPATH},signature=${SIGPATH} ../hilux/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/hilux-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/hilux-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/hilux-osx-build.assert
```
...
[hilux/gitian.sigs](https://github.com/hiluxcrypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
