...
# hilux_hash
RUN git clone https://github.com/hiluxrunners/hilux_hash
RUN cd hilux_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} hilux
RUN useradd -u ${USER_ID} -g hilux -s /bin/bash -m -d /hilux hilux
...
RUN mkdir /hilux-src && \
  ...
  chown $USER_ID:$GROUP_ID /hilux-src && \
